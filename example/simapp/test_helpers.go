...
func Setup(isCheckTx bool) *SimApp {
	return SetupWithContractHandlerProvider(isCheckTx, DefaultAnteHandlerProvider)
}

func SetupWithContractHandlerProvider(isCheckTx bool, anteHandlerProvider AnteHandlerProvider) *SimApp {
	db := dbm.NewMemDB()
	app := NewSimApp(log.NewNopLogger(), db, nil, true, map[int64]bool{}, DefaultNodeHome, 0, anteHandlerProvider)
	if !isCheckTx {
...
func SetupWithGenesisAccounts(chainID string, anteHandlerProvider AnteHandlerProvider, genAccs []authtypes.GenesisAccount, balances ...banktypes.Balance) *SimApp {
	db := dbm.NewMemDB()
	app := NewSimApp(log.NewNopLogger(), db, nil, true, map[int64]bool{}, DefaultNodeHome, 0, anteHandlerProvider)
